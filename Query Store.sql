-- Enable Query Store in Required DB
...
BEGIN
    IF NOT EXISTS (SELECT name FROM sys.all_objects WHERE UPPER(name) = 'QUERY_STORE_AWR')
        BEGIN
            SELECT Txt.query_sql_text, pl.plan_id, qry.* INTO [dbo].[query_store_awr]  
    ...
    ELSE
        BEGIN
            INSERT INTO [dbo].[query_store_awr]
            SELECT a.* 
    		FROM (
